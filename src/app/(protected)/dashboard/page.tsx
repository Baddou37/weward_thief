  ...
  return (
    <div className="p-4 lg:p-6 max-w-5xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500">{total} entrée{total > 1 ? 's' : ''} dans la base</p>
      </div>
      ...
      <div className="mt-4 space-y-4 lg:hidden">
        {thieves?.map((thief: Thief) => (
          <Link key={thief.id} href={`/voleurs/${thief.id}`}>
                ...
                <div className="min-w-0">
                  ...
                  {(thief.facebook_first_name || thief.facebook_last_name) && (
                    <p className="text-sm text-gray-500 truncate">
                      {[thief.facebook_first_name, thief.facebook_last_name].filter(Boolean).join(' ')}
                    </p>
                  )}
                  {thief.arnaque_type && (
                    <p className="text-xs text-gray-400 mt-1 truncate">{thief.arnaque_type}</p>
                  )}
                </div>
                <StatusBadge status={thief.status} />
              </div>
              <p className="text-xs text-gray-400 mt-2">{formatDate(thief.created_at)}</p>
            </div>
          </Link>
        ))}
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Pseudo(s) Weward</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Nom Facebook</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Type d&apos;arnaque</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {thieves?.map((thief: Thief) => (
                <tr key={thief.id} className="hover:bg-gray-50 cursor-pointer transition-colors">
                  <td className="px-4 py-3">
                    <Link href={`/voleurs/${thief.id}`} className="block">
                      <span className="font-medium text-gray-900">
                        {thief.weward_pseudos.length > 0 ? thief.weward_pseudos.join(', ') : '—'}
                      </span>
                    </Link>
                  </td>
